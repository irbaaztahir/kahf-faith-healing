...
import { Compass, Calendar, User as UserIcon } from "lucide-react";
...
export const Route = createFileRoute("/client")({
  head: () => ({ meta: [{ title: "Client dashboard — Kahf" }] }),
...
function ClientDashboard() {
  ...
  if (role !== "client") return <Navigate to="/therapist" replace />;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto grid max-w-6xl gap-6 px-6 py-12 sm:grid-cols-3">
        <PortalCard
          ...
          description="Browse verified Muslim therapists and book a session."
        />
        <PortalCard
          to="/dashboard"
          icon={Calendar}
          title="My Sessions"
          description="See upcoming bookings and review past sessions."
        />
        <PortalCard
          to="/settings"
          icon={UserIcon}
          title="My Profile"
          description="Update your details and privacy preferences."
        />
      </div>
    </div>
  );
